...
Overview、資產清冊、投資摘要及資產負債表共用相同的投資觀測 reconciliation。跨來源持倉只會在資料帶有相同 `economicSecurityId` 後合併。`complete` 觀測優先於同組 `subset`；若同組皆為 subset，價值會在幣別一致且資料完整時相加。系統不以股票代號推斷重疊。來源事實與 provenance 保留在各自的持倉列，明細可顯示來源拆分，但摘要不會重複計入來源子集。
