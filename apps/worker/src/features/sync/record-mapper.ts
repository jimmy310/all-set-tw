...
export function investmentPositionRecord(
  ...
  return {
    ...
    payload: {
      ...
      underlying_symbol: normalized.underlyingSymbol ?? null,
      expiration_date: normalized.expirationDate ?? null,
      strike_price: normalized.strikePrice ?? null,
      option_right: normalized.optionRight ?? null,
      contract_multiplier: normalized.contractMultiplier ?? 1,
      contract_symbol: normalized.contractSymbol ?? null,
      option_mark_price: normalized.optionMarkPrice ?? null,
      economic_security_id: normalized.economicSecurityId ?? null,
      observation_coverage: normalized.observationCoverage ?? "complete",
...
export function investmentTransactionRecord(
  ...
  return {
    ...
    payload: {
      ...
      currency: transaction.currency || "TWD",
      underlying_symbol: transaction.underlyingSymbol ?? null,
      expiration_date: transaction.expirationDate ?? null,
      strike_price: transaction.strikePrice ?? null,
      option_right: transaction.optionRight ?? null,
      contract_symbol: transaction.contractSymbol ?? null,
      external_contract_id: transaction.externalContractId ?? null,
      raw_payload: JSON.stringify(transaction.raw ?? transaction),
      created_at: now,
      updated_at: now,
    },
